...
#             NOVAS VIEWS PARA AS TELAS ADMIN
# View para a página de Configurações
def pagina_configuracoes(request):
    # A lógica para buscar a loja continua a mesma
    loja = Loja.objects.first()

    # NOVO: Verifica se o formulário foi enviado (método POST)
    if request.method == 'POST':
        ...
        # Atualiza o objeto 'loja' com os novos dados
        loja.nome = nome
        ...
        # Redireciona de volta para a mesma página para mostrar os dados atualizados
        return redirect('configuracoes')

    # Se não for POST (ou seja, a primeira vez que a página é carregada),
    # a lógica continua a mesma de antes.
    context = {
        'loja': loja
    }
...
# View para a página de Equipes
def pagina_equipes(request):
    # Por enquanto, só vai renderizar a página HTML.
    context = {}
    return render(request, 'core/equipes.html', context)
